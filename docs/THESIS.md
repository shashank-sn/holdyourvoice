...
The source product also explored feedback-derived voice memory. This repository includes a narrower local form: verification is read-only, while explicit or separately approved learning records resolved finding IDs against a fingerprint of the writer's portable profile. The next brief can prioritize those approved repairs. It is inspectable, bounded, stored only on the user's machine, and contains no draft text, candidate text, embeddings, client profiles, cloud storage, accounts, or telemetry.
