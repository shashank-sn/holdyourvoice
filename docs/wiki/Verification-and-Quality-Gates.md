...
`final-check` remains the profile-free hidden-Unicode output gate. `delivery-check` is a separate opt-in offline check for placeholders, likely credential-shaped text, local Markdown targets, and citation IDs supplied by a local policy. It does not fetch URLs or prove facts. Use `dispositions` for a compatibility-safe normalized view: `block` preserves gate behavior, `review` needs an editor decision, and `signal` is advisory. An optional surface-policy JSON file can map a stable finding ID to a local disposition without modifying the original profile or legacy report.
