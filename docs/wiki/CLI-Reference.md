...
`profile` needs two or more samples and writes only the profile path you name. `analyze` returns independent JSON reports. `rewrite-prompt` prints markdown and never calls a model. A passing `verify` records resolved finding IDs by default under local profile-scoped learning state; it never stores either text. `learning show`, `add`, and `clear` inspect, add, or remove that profile's local state. `verify` returns `2` when the gate fails and `1` for usage or runtime errors. `patterns` prints the exact executable deterministic rules.
