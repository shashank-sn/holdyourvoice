...
test('publishes the same normalized reconciled catalog and version through CLI and MCP', () => {
  ...
  assert.equal(cliCatalog.version, '3.5.0-local.1');
  assert.equal(cliCatalog.rules.length, 171);
...
test('agent list enumerates every command as one portable package', () => {
  ...
  for (const id of ['hyv-profile', 'hyv-score', 'hyv-ingest', 'hyv-analyze', 'hyv-verify', 'hyv-mcp', 'hyv-patterns', 'hyv-strict-check']) {
